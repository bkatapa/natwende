...
public abstract class MessageHelper<T extends BaseVO> implements Serializable {    
	...
	public static final void updateComponent(String componentId) {
		updateComponents(java.util.Collections.singleton(componentId));
	}
	
	public static final void updateComponents(String[] componentIds) {
		PrimeFaces.current().ajax().update(componentIds);
	}
	
	public static final void updateComponents(java.util.Collection<String> componentIds) {
		PrimeFaces.current().ajax().update(componentIds);
	}
	...
	public static final void showMessageDialog(FacesMessage msg, boolean dismissable) {
		PrimeFaces.current().dialog().showMessageDynamic(msg, dismissable);
	}
}
