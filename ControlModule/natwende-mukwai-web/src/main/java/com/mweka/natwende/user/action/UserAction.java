...
public class UserAction extends MessageHelper<UserVO> {
	...
	public void updateEntity() {
		try {
			saveEntity();
			RequestContext.getCurrentInstance().execute("PF('var_UserEditDlg').hide();");
			RequestContext.getCurrentInstance().update("@widgetVar(var_UserEditDlg)");
	...
	public StreamedContent getPreviewImage(UserVO user) throws IOException {
		...
		return new DefaultStreamedContent(user.getProfilePicAsStream());
	}
	...
	private void upload() {
		if (file != null) {
			selectedEntity.setProfilePic(file.getContents());
