...
public class OperatorSettingsAction extends AbstractActionBean<OperatorSettingsVO> {
	...
	public List<OperatorSettingsVO> getListFromFacade() {
		return null; //serviceLocator.getTenantSettingsDataFacade().getBySearch(searchVO);
	}
	...
	public OperatorSettingsVO facadeUpdate(OperatorSettingsVO dataItem) throws Exception {
		...
		if (actionMode == UserActionMode.CREATE) {
			utils.showInfoMessageDlg("Settings created successfully");
		}
		else if (actionMode == UserActionMode.EDIT) {
			utils.showInfoMessageDlg("Settings updated successfully");
		} 
		else {
			utils.showInfoMessageDlg("Settings saved successfully");
		}
		return result;
	}
	...
	protected void facadeDelete(OperatorSettingsVO dataItem) {
		if (dataItem.getId() > -1L) {
			try {
				...
				log.debug(e);
				utils.showErrorMessageDlg("There was a problem deleting the settings: " + e.getMessage());
			}
		}
		else {
			utils.showErrorMessageDlg("There settings you are requesting to delete do not exist on the database");
		}
	}
